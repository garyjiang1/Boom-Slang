...
Boomslang is a general-purpose imperative programming language inspired by Python and Java. It aims to combine the ease of use of Python with the type safety of Java. Boomslang is strongly and statically typed, has no type inferencing, and supports object-oriented programming without requiring that all functions be part of objects. Written in OCaml, Boomslang compiles to LLVM IR.

Boomslang uses syntactically significant whitespace instead of curly braces and semicolons. In addition to static typing, it includes quality of life features such as built-in support for data classes (automatic constructor generation and meaningful automatic string conversion), a new syntax for loops, and a better operator overloading syntax.

Boomslang (or something similar) means "tree snake" in several Germanic languages. The name pays homage to Python and ends in "lang," evoking the word "language" as in "programming language."

## Compiling Programs

To compile a program, use the `boomc` utility. For example, `./boomc helloworld.boom` will compile `helloworld.boom` to LLVM IR. Using the `-r` flag with `boomc` as in `./boomc -r helloworld.boom` will compile and execute the program in one step.

To compile the compiler, run `make` inside the `src` directory. This produces a `boomslang.native` program that can generate Graphviz representations of the abstract syntax tree or semantically checked abstract syntax tree, as well as LLVM IR.

To run the test suite, execute `make test` inside the `src` directory.
